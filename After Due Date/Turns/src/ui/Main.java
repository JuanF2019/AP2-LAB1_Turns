...
public class Main {
	...
	public static TurnManager registerUser(TurnManager tm){
		...
		for (int i =0; i < charArrayData.length; i++) {
			stringArrayData[i] = Character.toString(charArrayData[i]);
		}
		for (int i =0; i < stringArrayData.length; i++) {
			if(stringArrayData[i].equals(";")) {
				stringArrayData[i] = "; ";
			}
		}
	...
	public static TurnManager assignTurn(TurnManager tm){
		...
		if (splitData.length == 2 && check ) {			
			try {
				turn = tm.assignTurn(splitData[0], splitData[1]);
				System.out.println("Assigned turn for user " + splitData[0].toUpperCase() + " " + splitData[1] + " is: "+ turn);
